...
const SAMPLE_APPLICATION: ApplicationRecord = {
  id: "1",
  ...
  summary:
    "We're looking for a Senior Frontend Engineer to join our platform team. You'll be working on our next-generation design system and component library, building performant and accessible user interfaces that serve millions of users. The ideal candidate has deep experience with React, TypeScript, and modern CSS, along with a passion for developer experience and code quality.",
  requirements: [
    "5+ years of experience with React and TypeScript",
    "Strong understanding of web accessibility (WCAG 2.1)",
    "Experience building and maintaining design systems",
    "Proficiency with modern CSS (Tailwind, CSS-in-JS)",
  ...
  potentialImprovements: [
    "Consider adding more GraphQL experience to your resume",
    "Highlight any design system work you've done",
    "Include specific metrics about performance improvements",
  ],
  fitScore: 82,
  fitReasoning: [
    "Strong React/TypeScript experience matches requirements",
    "Design system experience aligns well with the role",
    "Accessibility knowledge is a plus for this position",
    "Location preference matches (hybrid-friendly)",
  ],
  status: "APPLIED",
  notes: "Reached out to the hiring manager on LinkedIn. Waiting to hear back about next steps.",
  createdAt: "2024-03-10T14:30:00Z",
  updatedAt: "2024-03-12T09:15:00Z",
};

function getFitScoreColor(score: number) {
  if (score >= 75) return { bg: "bg-neon-green/20", text: "text-neon-green", border: "border-neon-green/40", bar: "bg-neon-green" };
  if (score >= 50) return { bg: "bg-neon-orange/20", text: "text-neon-orange", border: "border-neon-orange/40", bar: "bg-neon-orange" };
  return { bg: "bg-destructive/20", text: "text-destructive", border: "border-destructive/40", bar: "bg-destructive" };
}
...
export default function JobDetailPage() {
  // In a real app, you'd fetch this based on the route param
  const application = SAMPLE_APPLICATION;
    ...
    <div className="min-h-screen bg-background cyber-grid relative">
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="mb-6 border-border bg-card/80 backdrop-blur-sm dark:neon-border-cyan dark:bg-card/60">
          <CardContent className="p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              {/* Status Dropdown */}
              <Select value={status} onValueChange={(value) => setStatus(value as ApplicationRecord["status"])}>
                <SelectTrigger
                  className={cn(
        ...
        <Card className="mb-6 border-border bg-card/80 backdrop-blur-sm dark:bg-card/60">
          ...
          <CardContent>
            <p className="text-muted-foreground leading-relaxed">{application.summary}</p>
          </CardContent>
        </Card>
        ...
        <Card className="mb-6 border-border bg-card/80 backdrop-blur-sm dark:bg-card/60">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-foreground">Why this score?</h4>
              <ul className="space-y-2">
                {application.fitReasoning.map((reason, index) => (
                  ...
                  >
                    <CheckCircle2 className={cn("size-4 mt-0.5 shrink-0", scoreColors.text)} />
                    {reason}
                  </li>
                ))}
              </ul>
            ...
            <CardContent>
              {isPerfectMatch ? (
                <div className="flex items-center gap-3 rounded-lg border border-neon-green/40 bg-neon-green/10 p-4 dark:neon-glow-cyan">
                  ...
                  <div>
                    <p className="font-medium text-neon-green">Perfect Match!</p>
                ...
                <ul className="space-y-3">
                  {application.potentialImprovements.map((improvement, index) => (
                    <li
                      key={index}
